...
public class EventRepository implements PanacheRepositoryBase<EventEntity, Integer> {
    ...
    public Uni<List<EventEntity>> listWithPaging(String sortBy, String orderBy, Integer page, Integer size) {

        if (page < 1) page = 1;
        if (size < 1) size = 5;
        page = (page - 1) * size;
        if (sortBy == null) sortBy = "eventid";
        Sort.Direction direction = Sort.Direction.Ascending;
        if (Objects.equals(orderBy, "desc")) direction = Sort.Direction.Descending;

        return EventEntity.findAll(Sort.by(sortBy).direction(direction)).page(page, size).list();
    }
